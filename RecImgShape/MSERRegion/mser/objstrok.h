...
void CptPointPairParameterLength(vector<PointPair>&PairVec, PairInfoStat&pairInfo)
{
   ...
   Loopi(lengthVec.size())
   {
	   double diff = fabs((lengthVec[i] - pairInfo.medianVal)/pairInfo.medianVal);
	   if(diff<0.2)
		   cnt++;
   }
...
void ObjStroke::CalculateObjStroke(CharRegion & Region,vector<PointPair>&pairVec)
{
	...
	Loopi(Region.contourPts.size())
	{
	  ...
	  for(step =1; step < maxstepSize; step++)
	  {
         ...
         float nextX = round(startX + Pt.dx * step);
         float nextY = round(startY + Pt.dy * step);
         ...
         if(LableMap[Posoffset]!=startLb)
		 {
			...
			if(step == 1)break;
            step -=1;
			nextX = round(startX + Pt.dx * step); nextY = round(startY + Pt.dy * step);
			Posoffset = nextY * picWidth + nextX;
			...
			double AngleDiff = fabs(fabs(Pt.angle - PtT.angle) - 180.0);
	        if( AngleDiff>35.0)
				 break;
