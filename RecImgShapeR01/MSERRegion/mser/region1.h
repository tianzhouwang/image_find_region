...
//---------------------------------------------------------------------     	
     ...
     void label_image(bool K8_connectivity);
     BYTE*imgpt;
     int *labelimg;
...
int ConnectedComponents::new_label() 
{
 ...
 void ConnectedComponents::label_image(bool K8_CONNECTIVITY)
{
    BYTE *img = img_buffer_pt;
	imgpt=img;
    BYTE *row=0;
    BYTE *last_row = 0;
    int crtval;
...
{    
  ...
  //    pt.y=result[i]/scan_width;
	...
	{	 
		int* line_pt = &labelimg[j*scan_width];
		for(i=left;i<=right;i++)
		{
		  line_pt[i] = labels[root_of(labelimg[i])].tag; 
		}
