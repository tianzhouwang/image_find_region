	...
	for( i=0;i<Cp.ResultVec.size();i++)
	{
		...
		Loopj(Cp.ResultVec[i].PtVec.size())
		{
			CPic.SigDot(Cp.ResultVec[i].PtVec[j].x,
				        Cp.ResultVec[i].PtVec[j].y);
			
			 RPoint Pt;
			 Pt.x = Cp.ResultVec[i].PtVec[j].x;
			 Pt.y = Cp.ResultVec[i].PtVec[j].y;
		...
		if(fabs( Cp.ResultVec[i].angle)>5.0)
		{
			CPic.DrawRotCentRectangle(Cp.ResultVec[i].x, Cp.ResultVec[i].y,Cp.ResultVec[i].Dx, Cp.ResultVec[i].Dy,
							- Cp.ResultVec[i].max_rot_x, - Cp.ResultVec[i].max_rot_y, 
							  Cp.ResultVec[i].max_rot_x,   Cp.ResultVec[i].max_rot_y);
							  
			  CPic.DrawTkLine(Cp.ResultVec[i].x ,  Cp.ResultVec[i].y,
		                  Cp.ResultVec[i].x +  Cp.ResultVec[i].Dx * 80.0 ,
						  Cp.ResultVec[i].y +  Cp.ResultVec[i].Dy * 80.0 ,2.5);
		}
		
		
		//else
			//CPic.Rectangle(Cp.ResultVec[i].left , Cp.ResultVec[i].top, 
		    //               Cp.ResultVec[i].right, Cp.ResultVec[i].bottom);
	}
